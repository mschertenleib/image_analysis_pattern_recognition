...
def check_df(df: pd.DataFrame, df_name: Union[str, None] = None):
    ...
    if len(non_int_cols):
        err_strs.append(
            TermFormat.format(f"Non-int columns: {non_int_cols.index.tolist()}", TermFormat.RED)
        )
    ##
    ...
    if len(neg_rows_mask):
        err_strs.append(
            TermFormat.format(
                f"IDs with negative values: {neg_rows_mask.index.tolist()}", TermFormat.RED
            )
        )
    ##
    if len(err_strs):
...
def match_submission(local_sub_path: str, kaggle_sub_path: str):
    ...
    if num_diff == 0:
        ...
        print(
            TermFormat.format(
                f"{num_diff} differences found (NaN = no difference):", TermFormat.RED
            )
        )
...
if __name__ == "__main__":
    ...
    check_parser = subparsers.add_parser(
        "check", help="checks whether the submission file is valid"
    )
    check_parser.add_argument(
        "--path",
        type=str,
        default="submission.csv",
        help="path of submission file to check (default: 'submission.csv')",
    )
    ...
    match_parser.add_argument(
        ...
        type=str,
        default="submission.csv",
        help="path of local submission file (default: 'submission.csv')",
    )
    match_parser.add_argument(
        "--kaggle",
        type=str,
        default="kaggle.csv",
        help="path of Kaggle submission file (default: 'kaggle.csv')",
    )
